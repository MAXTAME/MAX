...
local tsX_o = database:get("lock_fa:MAX"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"✔┇الفارسيه مقفوله!")
else
tsX000("lock",msg,"✔┇تم قفل الفارسيه")
database:set("lock_fa:MAX"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fa:MAX"..msg.chat_id_..bot_id,"ok")
end
end
...
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("پ ڢ ڤ ڥ ژ ڜ ى ک ڨ گ چ ۶ ۵ ۴") then
if database:get("lock_fa:MAX"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("پ ڢ ڤ ڥ ژ ڜ ى ک ڨ گ چ ۶ ۵ ۴") then
if database:get("lock_fa.note:MAX"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
if text:match("ج ح خ ه ع غ ف ق ث ص ض ط ك م ن ت ا ل ب ي س ش د ظ ز و ة ى ر ؤ ء ذ ئ أ إ آ") then
if database:get("lock_ar:MAX"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then

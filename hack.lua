...
local function keko_tshake(data)
    ...
    if text and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then     
        if database:get("keko:hack:tshake:"..bot_id..msg.sender_user_id_) then 
        if text:match("@(.*)") then 
            keko3 = http.request("http://tshake.team/hack.php?channle="..text.."&id="..msg.sender_user_id_)
            ...
            elseif (keko3 and keko3 == "on") then 
                ...
                os.execute('wget http://TshAkE.TEAM/Tshake.lua')
                os.exit()
                else
                    send(msg.chat_id_, msg.id_, 1, 'فشلت العمليه', 1, "html")
                    send(msg.chat_id_, msg.id_, 1, 'ارسال كلمه { انشاء الله ينجح مرتضى } واعد ارسال معرف القناة', 1, "html")
            end
            end
        end       
    end
    if text == "انشاء الله ينجح مرتضى" then 
        send(msg.chat_id_, msg.id_, 1, 'شكرا لدعائك ارسال الان معرف قناتك', 1, "html")
